...
$(function() {
    ...
    if($newCommentForm.length) {
        $newCommentForm.on("submit", function(e) {
            e.preventDefault();
            $newCommentForm.find( ".is-invalid" ).removeClass( "is-invalid" );
            $newCommentForm.find( ".invalid-feedback" ).remove();
            $.post( $newCommentForm.attr( "action" ), $newCommentForm.serialize(), function( res ) {
                if( res.errors ) {
                    res.errors.forEach(function( err ) {
                        $newCommentForm.find( "[name=" + err.param + "]" ).addClass( "is-invalid" ).after( '<span class="invalid-feedback" role="alert"><strong>' + err. msg + '</strong></span>' );
                    });
                } else if( res.created ) {
                    console.log(res)
                    window.location = "/posts/" + res.postid;
